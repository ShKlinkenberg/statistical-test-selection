...
# Display Markdown:
input <- system.file("test-selection-scheme.Rmd", package = "mindr")
# file.show(input)
input_txt <- readLines("test-selection-scheme.Rmd", encoding = "UTF-8")
markmap(input_txt)
...
```{mermaid}
flowchart LR
  A{Number of outcome variables} --> B(One)
  A --> 2(More than one)
  B --> C{Decision}
  C --> D[Result one]
  C --> E[Result two]
